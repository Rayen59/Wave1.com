...
def init_db():
    ...
    try:
        ...
        pass
    try:
        c.execute("ALTER TABLE messages ADD COLUMN forwarded_from_id INTEGER;")
    except sqlite3.OperationalError:
        pass
    try:
        c.execute("ALTER TABLE groups ADD COLUMN avatar TEXT DEFAULT '';")
    except sqlite3.OperationalError:
        pass
    try:
        c.execute("ALTER TABLE groups ADD COLUMN password_hash TEXT DEFAULT '';")
    except sqlite3.OperationalError:
        pass
    conn.commit()
    conn.close()
...
@auth_required
def change_member_role(gid):
    data = request.get_json(force=True) or {}
    user_id = data.get("user_id")
    new_role = data.get("role")
    
    if not user_id or new_role not in ("member", "moderator", "admin"):
        return jsonify({"error": "Invalid role or missing user_id"}), 400
    ...
    target = db.execute(
        "SELECT role FROM group_members WHERE group_id=? AND user_id=?", (gid, user_id)
    ).fetchone()
    if not target:
    ...
    socketio.emit("group_member_updated", {
        "group_id": str(gid),
        "action": "role_changed",
        "target_id": user_id,
...
def remove_member(gid):
    ...
    db.execute(
        "DELETE FROM group_members WHERE group_id=? AND user_id=?", (gid, user_id)
    )
    db.commit()
...
@app.route("/api/groups/<int:gid>/restrict", methods=["POST"])
@auth_required
def restrict_member(gid):
    data = request.get_json(force=True) or {}
    user_id = data.get("user_id")
    ...
    if not caller or caller["role"] != "admin":
        return jsonify({"error": "Only admins can restrict members"}), 403
    ...
    # Store restriction in a new table or use a simple approach
    # For simplicity, we'll store it in the group_members table with a restricted_until column
    try:
        db.execute("ALTER TABLE group_members ADD COLUMN restricted_until REAL")
    except sqlite3.OperationalError:
        pass
    
    restricted_until = time.time() + duration
    db.execute(
        "UPDATE group_members SET restricted_until=? WHERE group_id=? AND user_id=?",
        (restricted_until, gid, user_id)
    )
    db.commit()
...
@app.route("/api/groups/<int:gid>/unrestrict", methods=["POST"])
@auth_required
def unrestrict_member(gid):
    data = request.get_json(force=True) or {}
    user_id = data.get("user_id")
    ...
    if not caller or caller["role"] != "admin":
        return jsonify({"error": "Only admins can unrestrict members"}), 403
...
def sio_typing(data):
    ...
    if chat_type == "dm":
        ...
        emit("typing", {"user_id": uid}, room=f"group:{target}")
...
def sio_call_answer(data):
    ...
    if target is None or not answer:
        return
    emit("call_answer", {"answer": answer}, room=f"user:{target}")
...
def sio_call_ice(data):
    ...
    if target is None or not candidate:
        return
    emit("call_ice_candidate", {"candidate": candidate}, room=f"user:{target}")
...
def sio_call_reject(data):
    ...
    if target is None:
        return
    emit("call_reject", {}, room=f"user:{target}")
...
def sio_call_end(data):
    ...
    if target is None:
        return
    emit("call_end", {}, room=f"user:{target}")
